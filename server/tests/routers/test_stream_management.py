...
def test_create_stream_valid(valid_stream):
    with patch("src.routers.stream_management.dataset_map") as mock_dataset_map, patch(
        ...
        mock_dataset_map.__getitem__.return_value = mock_dataset_instance
        mock_dataset_instance().load.return_value = "data"
        ...
        # mock Evaluator Streamer
        mock_evaluator_streamer_instance = MagicMock()
        mock_evaluator_streamer.return_value = mock_evaluator_streamer_instance

        # mock return values of MetricEntry calls
        mock_metric_entry.side_effect = [
            "PrecisionK",
            "RecallK",
        ]  # return values of mock metric entry

        mock_write_to_db.return_value = "336e4cb7-861b-4870-8c29-3ffc530711ef"
        ...
        mock_sliding_window_instance.split.assert_called_once_with("data")
        ...
        mock_evaluator_streamer.assert_called_once_with(
            ["PrecisionK", "RecallK"], mock_sliding_window_instance, 10
        )

        mock_write_to_db.assert_called_once_with(mock_evaluator_streamer_instance)
...
def test_get_stream_valid(mock_evaluator_stream):
    with patch(
        "src.routers.stream_management.get_evaluator_stream_from_db",
        return_value=mock_evaluator_stream,
    ) as mock_get_from_db:
...
def test_get_stream_invalid_uuid():
    response = client.get("/streams/invalid_uuid")
    assert response.status_code == 400
    assert response.json() == {"detail": "Invalid UUID format"}


def test_get_stream_stream_not_found():
    with patch(
        "src.routers.stream_management.get_evaluator_stream_from_db", return_value=None
    ) as mock_get_from_db:
        response = client.get("/streams/336e4cb7-861b-4870-8c29-3ffc530711ef")
...
def test_get_stream_failed_to_get_from_db():
    with patch(
        "src.routers.stream_management.get_evaluator_stream_from_db",
